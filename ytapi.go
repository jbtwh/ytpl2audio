...
const developerKey = "AIzaSyAQDML9xWY7OAcV1IGit9ueyAO-MlgcLz4"
...
func printPlaylistItemsListResults(response *youtube.PlaylistItemListResponse) {
	for _, item := range response.Items {
		fmt.Println(item.Snippet.ResourceId.VideoId, ": ", item.Snippet.Title)
	}
}

func playlistItemsListByPlaylistId(service *youtube.Service, part string, maxResults int64, playlistId string) *youtube.PlaylistItemListResponse {
...
func idsFromPlaylist(id string) []string {

	service, err := youtube.New(netClient)
	handleError(err, "Error creating YouTube client")

	var ids []string
	items := playlistItemsListByPlaylistId(service, "snippet,contentDetails", 50, id).Items
	for _, i := range items {
		if !strings.Contains(i.Snippet.Title, "Private") {
			ids = append(ids, i.Snippet.ResourceId.VideoId)
		}
	}
